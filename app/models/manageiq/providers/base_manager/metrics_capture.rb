class ManageIQ::Providers::BaseManager::MetricsCapture
  ...
  # @param start_time [String, nil] Start time
  # @param end_time   [String, nil] Ending time
  # [{
  #   :ems_id         => ems_id,         # "2"
  ...
  def fix_capture_start_end_time(interval_name, object = nil, start_time = nil, end_time = nil)
    ...
    if interval_name == 'historical'
      ...
      start_time ||= object ? object.last_perf_capture_on : ems.last_metrics_success_date
      # For hourly on the first capture, we don't want to get all of the
      #   historical data, so we shorten the query
      start_time ||= HOURLY_METRICS_DURATION.ago.utc if interval_name == 'hourly'
    end
    [start_time, end_time]
  end
